var input = `bot 123 gives low to bot 191 and high to bot 162
...
bot 6 gives low to bot 27 and high to bot 37`.split("\n");
...
var init = function(input){
    ...
    _.each(input, function(line){
       ...
       if(line[0] == "b"){
           ...
           var bot = {
               ...
               type : "bot",
               low : function(){ 
                    return nodes[cmd[5]][idx(cmd, 6)];
               },
               high : function(){ 
                    return nodes[cmd[10]][idx(cmd, 11)]; 
               },
               stack :[],
               give : function(){
                   console.log(this.id, "gives", _.min(this.stack), "to", this.low().type, this.low().id, "and", _.max(this.stack), "to", this.high().type, this.high().id);
                   this.low().take(_.min(this.stack));
                   this.high().take(_.max(this.stack));
                   this.stack = [];
               },
               take : function(chip){
                   console.log(this.id, "takes", chip);
                   this.stack.push(chip);
                   if(this.stack.length ==2 ){
                       if(_.min(this.stack) == 17 && _.max(this.stack) == 61){
                           console.log("HERE", this.id, this.stack);
                       }
                       this.give();
                   }
               },
               print : function(){
                   console.log("id:", this.id, "stack:", this.stack, "low:", this.low(), "high:", this.high());
               }
           };
           (nodes.bot)[cmd[1]] = bot;
           if(cmd[5] == "output"){ 
                nodes.output[idx(cmd, 6)] = { 
                    id : idx(cmd, 6), 
                    type : "output",
                    stack : [],
                    take : function(chip){
    ...
    _.each(input, function(line){
       if(line[0] == "v"){
           var cmd = line.split(" ");
           (nodes.bot)[idx(cmd, 5)].stack.push(idx(cmd, 1));
       }
    });
};
...
var doit = function(input){
    ...
    for(i=0; i<nodes.output.length; i++){    
        console.log(nodes.output[i].stack);
    }    
};
