...
var spl = function(line){
    return line.split(/[\[\]]/g);
};
var gr = function(arr){
    return _.reduce(arr, function(memo, i){
        if((memo.ip.length + memo.hyper.length)%2 == 0){
            memo.ip.push(i);
        }else{
            memo.hyper.push(i);
        }
...
var doit = function(input){
    ...
    output = _.map(input, function(line){
        return {
            original : line,
            splitted : spl(line).join(" "),
            grouped : gr(spl(line)),
            print : function(){
                return [this.original, this.splitted, this.grouped.ip.join(" "), this.grouped.hyper.join(" ")].join("\n");  
            }
        };
    });
